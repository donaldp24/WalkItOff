...
@synthesize currentMonth,delegate,labelCurrentDate, animationView_A,animationView_B;
...
-(void)showNextMonth {
    ...
    UIView *animationHolder = [[UIView alloc] initWithFrame:CGRectMake(0, kVRGWeekDaysTopBarHeight, kVRGCalendarViewWidth, targetSize-kVRGWeekDaysTopBarHeight - kVRGCurrentLabelHeight)];
    ...
    if (self.animType == CalendarAnimateVertical)
    {
        if (hasNextMonthDays) {
    ...
    {
        animationView_B.frameX = animationView_A.frameX + animationView_A.frameWidth;
    ...
    [UIView animateWithDuration:.35
                     animations:^{
                         ...
                         {
                             animationView_A.frameX -= animationView_A.frameWidth;
...
-(void)selectNextDate {
    ...
    NSDate *nextDate = [self.selectedDate offsetDay:1];
    if ([nextDate month] > [self.selectedDate month])
    {
...
-(void)showPreviousMonth {
    ...
    {
        animationView_B.frameX = animationView_A.frameX - animationView_B.frameWidth;
    }
...
-(float)calendarHeight {
    ...
    return kVRGWeekDaysTopBarHeight + [self numRows]*(kVRGCalendarViewDayHeight+2)+1 + kVRGCurrentLabelHeight;
}
...
{       
    ...
    CGFloat arrowY = kVRGWeekDaysTopBarHeight + gridHeight;
    CGRect rectArrowLeft = CGRectMake(0, arrowY, 50, 40);
    CGRect rectArrowRight = CGRectMake(self.frame.size.width-50, arrowY, 50, 40);
    ...
    if (CGRectContainsPoint(rectArrowLeft, touchPoint)) {
        ...
        [self selectNextDate];
        return;
    } else if (CGRectContainsPoint(self.labelCurrentDate.frame, touchPoint)) {
    ...
    if (touchPoint.y > /*kVRGCalendarViewTopBarHeight*/ kVRGWeekDaysTopBarHeight) {
        float xLocation = touchPoint.x;
        float yLocation = touchPoint.y-/*kVRGCalendarViewTopBarHeight*/ kVRGWeekDaysTopBarHeight;
...
- (void)drawRect:(CGRect)rect
{
    ...
    // Weekdays
    CGRect rectangleWeekdays = CGRectMake(0, 0, self.frame.size.width, kVRGWeekDaysTopBarHeight);
    CGContextAddRect(context, rectangleWeekdays);
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0xf9f8f8"].CGColor);
    ...
    CGContextSetFillColorWithColor(context,
                                   [UIColor colorWithHexString:@"0x3598db"].CGColor);
    ...
    NSMutableDictionary *attributes = [@{
                                         NSForegroundColorAttributeName: [UIColor colorWithHexString:@"0x3598db"],
    ...
    for (int i =0; i<[weekdays count]; i++) {
        ...
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth+2), 8, kVRGCalendarViewDayWidth+2, kVRGWeekDaysTopBarHeight) withAttributes:attributes];
    }
    ...
    //Grid background
    float gridHeight = numRows*(kVRGCalendarViewDayHeight+2)+1;
    CGRect rectangleGrid = CGRectMake(0, kVRGWeekDaysTopBarHeight, self.frame.size.width,gridHeight);
    ...
    //Draw days
    CGContextSetFillColorWithColor(context, 
                                   [UIColor colorWithHexString:@"0x2d3e50"].CGColor);
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        int targetY = /*kVRGCalendarViewTopBarHeight + */ kVRGWeekDaysTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2);
        
        // BOOL isCurrentMonth = NO;
        if (i<firstWeekDay) { //previous month
            targetDate = (prevMonthNumDays-firstWeekDay)+(i+1);
            NSString *hex = (isSelectedDatePreviousMonth) ? @"0x383838" : @"0xb8c8c8";
            ...
            NSString *hex = (isSelectedDateNextMonth) ? @"0x383838" : @"0xb8c8c8";
            CGContextSetFillColorWithColor(context, 
                                           [UIColor colorWithHexString:hex].CGColor);
        } else { //current month
            // isCurrentMonth = YES;
            targetDate = (i-firstWeekDay)+1;
            NSString *hex = (isSelectedDatePreviousMonth || isSelectedDateNextMonth) ? @"0xb8c8c8" : @"0x383838";
            CGContextSetFillColorWithColor(context, 
                                           [UIColor colorWithHexString:hex].CGColor);
        }
        ...
        if (selectedDate && i==selectedDateBlock) {
            CGRect rectangleGrid = CGRectMake(targetX+2,targetY+2,kVRGCalendarViewDayWidth-2,kVRGCalendarViewDayHeight-2);
            //CGContextAddRect(context, rectangleGrid);
            CGContextAddRoundRect(context, rectangleGrid, 5.0);
            //CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0x006dbc"].CGColor);
            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0xe84c3d"].CGColor);
            CGContextFillPath(context);
           ...
           // CGContextAddRect(context, rectangleGrid);
            CGContextAddRoundRect(context, rectangleGrid, 10.0);
            //CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0x383838"].CGColor);
            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0xffe7e5"].CGColor);
            CGContextFillPath(context);
    ...
    else
    {
        ...
        for (int i = 0; i<[self.markedDates count]; i++) {
            ...
            int targetY = /* kVRGCalendarViewTopBarHeight + */ kVRGWeekDaysTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2) + kVRGMarkTopMargin;
    ...
    [labelCurrentDate setBackgroundColor:[UIColor clearColor]];
    labelCurrentDate.text = [formatter stringFromDate:self.selectedDate];
    [labelCurrentDate sizeToFit];
    labelCurrentDate.frameX = roundf(self.frame.size.width/2 - labelCurrentDate.frameWidth/2);
    //labelCurrentDate.frameY = 10;
    [formatter release];
    labelCurrentDate.frameY = kVRGWeekDaysTopBarHeight + gridHeight + kVRGCurrentLabelTopMargin;
    
    // fill background
    CGRect rectangle = CGRectMake(0, kVRGWeekDaysTopBarHeight + gridHeight, self.frame.size.width, kVRGCurrentLabelHeight);
    CGContextAddRect(context, rectangle);
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0xf4f4f4"].CGColor);
    CGContextFillPath(context);

    // lines of current Date
    CGRect topLineRect = CGRectMake(0, kVRGWeekDaysTopBarHeight + gridHeight, self.frame.size.width, 1);
    CGContextAddRect(context, topLineRect);
    CGRect bottomLineRect = CGRectMake(0, kVRGWeekDaysTopBarHeight + gridHeight + kVRGCurrentLabelHeight - 1, self.frame.size.width, 1);
    CGContextAddRect(context, bottomLineRect);
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0x8a8a8a"].CGColor);
    CGContextFillPath(context);
    
    //Arrows
    int arrowSize = 12;
    int xmargin = 20;
    int ymargin = kVRGWeekDaysTopBarHeight + gridHeight + kVRGCurrentLabelTopMargin + 7;
...
-(UIImage *)drawCurrentState {
    float targetHeight = kVRGWeekDaysTopBarHeight + [self numRows]*(kVRGCalendarViewDayHeight+2) + kVRGCurrentLabelHeight +1;
    
    UIGraphicsBeginImageContext(CGSizeMake(kVRGCalendarViewWidth, targetHeight - kVRGWeekDaysTopBarHeight - kVRGCurrentLabelHeight));
    CGContextRef c = UIGraphicsGetCurrentContext();
    CGContextTranslateCTM(c, 0, -kVRGWeekDaysTopBarHeight);    // <-- shift everything up by 40px when drawing.
...
-(id)init {
    ...
    if (self) {
        ...
        isAnimating=NO;
        self.labelCurrentDate = [[UILabel alloc] initWithFrame:CGRectMake(34, 0, kVRGCalendarViewWidth-68, 40)];
        [self addSubview:labelCurrentDate];
        labelCurrentDate.backgroundColor=[UIColor whiteColor];
        labelCurrentDate.font = [UIFont fontWithName:@"HelveticaNeue-Bold" size:17];
        labelCurrentDate.textColor = [UIColor colorWithHexString:@"0x383838"];
        labelCurrentDate.textAlignment = UITextAlignmentCenter;
        
        self.animType = CalendarAnimateVertical;
