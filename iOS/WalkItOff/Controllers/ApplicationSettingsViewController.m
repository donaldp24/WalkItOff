...
@property (nonatomic, strong) IBOutlet UISwitch *dailyNotif;
@property (nonatomic, strong) IBOutlet UISwitch *notif;
...
- (void)viewDidLoad
{
    ...
    [self.dailyNotif setOn:settings.isDailyNotification];
    [self.notif setOn:settings.isNotificationWhenAllCalories];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0)
    {
        UnitSettingsViewController *vc = [self.storyboard instantiateViewControllerWithIdentifier:@"UnitSettingsViewController"];
...
{
    AppSettings *settings = [AppSettings sharedSettings];
    settings.isDailyNotification = self.dailyNotif.isOn;
    settings.isNotificationWhenAllCalories = self.notif.isOn;
    [settings save];
}
